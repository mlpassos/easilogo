...
Turtle.prototype.update = function() {
    if (this.visible && this.x >=0 && this.y >= 0 && this.x <= this.max_x && this.y <= this.max_y) {
        this.turtle.style.left = parseInt(this._left + this.x-5) + "px";
        this.turtle.style.top = parseInt(this._top + this.y-10) + "px";
        this.sprite.setAttribute('transform','rotate('+(this.angle)+' 10 10)');
    } else {
        this.turtle.style.left = "-20px";
        this.turtle.style.top = "-100px";
    }
    //if (this.active = false) {alert('inativo');}   
}
...
Turtle.prototype.crawl = function (d) {
    ...
    if (this.pen) {
        ...
        //alert('sending /draw?x='+~~(newx-250)+'&y='+~~(newy-200));
        $.get( '/draw?x='+(~~newx-250)+'&y='+(~~newy-200), function( data ) {
            // passing parameters now
        });
    } else {
...
Turtle.prototype.right = function(angle) {
    $("#turtle").css("transform","rotate(" + (this.angle + angle+90)  + "deg)");
    this.savestate();
...
Turtle.prototype.clean_ = function() {
    ...
    this.c.fillStyle = "rgb(238,238,238)";
